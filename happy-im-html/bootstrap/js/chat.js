    ...
    switch (data.type) {
        case TYPE_UPDATE_USERLIST_SYSTEM_MESSGAE:
            ...
            userList.append(
                ...
                '<div style="color: white;font-size: large">哈皮群聊室</div>' +
                '</div>');
            users.forEach(function (user) {
                ...
                userList.append(
                    ...
                    '<img id="redPoint-' + decodeURI(user) + '" class="img-circle" src="../img/redPoint.png" style="height: 10px;width: 10px;position: absolute;left: 60;display: none">' +
                ...
                appendString =
                    ['<div class="box" id="box-' + user + '" style="display: none">',
                        ...
                        '        </label>',
                        '        <div class="send"><button id="send-'+user+'" class="sendButton" onClick="sendMessageToUser(this.form.message.value, currentChatUserId)">发送</button></div>',
                        '    </form>',
                        '</div>'].join("");
                        
                repeatBox.append(appendString);
                $('#sendTextarea-'+user+'').bind('keyup', function(event) {
                    　　if (event.keyCode == 13) {
                        　　　$('#send-'+user+'').click();
                    　　}
                });
